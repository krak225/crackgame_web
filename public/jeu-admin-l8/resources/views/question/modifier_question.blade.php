...
<ul class="breadcrumb no-border no-radius b-b b-light pull-in"> 
	...
	<li class="active">Modification d'une question</li> 
</ul> 
...
<div class="panel panel-default"> 
	...
	<div class="step-content clearfix"> 
		<form enctype="multipart/form-data" id="form_add_question" method="post" class="form-horizontal" action="{{route('SaveModificationQuestion', $question->id)}}">
			...
			<div class="step-pane active" id="step1"> 
				...
				<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
					...
					<div class="col-md-9">
						<input maxlength="255" id="question_" type="text" class="form-control" name="question" value="{{ $question->question_fr }}" required autofocus>
					</div>
				</div>
				...
				<div class="form-group{{ $errors->has('proposition_1') ? ' has-error' : '' }}">
					...
					<div class="col-md-9">
						<input id="proposition_1" type="text" class="form-control" name="proposition_1" value="{{ $question->bonne_reponse }}" required>
					</div>
				</div>
				...
				<div class="form-group{{ $errors->has('proposition_2') ? ' has-error' : '' }}">
					...
					<div class="col-md-9">
						<input id="proposition_2" type="text" class="form-control" name="proposition_2" value="{{ $question->mauvaise_reponse_1 }}" required>
					</div>
				</div>
				...
				<div class="form-group{{ $errors->has('proposition_3') ? ' has-error' : '' }}">
					...
					<div class="col-md-9">
						<input id="proposition_3" type="text" class="form-control" name="proposition_3" value="{{ $question->mauvaise_reponse_2 }}" required>
					</div>
				</div>
				...
				<div class="form-group{{ $errors->has('categorie_id') ? ' has-error' : '' }}">
					...
					<div class="col-md-3">
						<select id="categorie_id" class="form-control" name="categorie_id" required>
							...
							<option @if($question->categorie_id == $categorie->categorie_id) selected @endif value="{{ $categorie->categorie_id }}">{{ $categorie->categorie_libelle }}</option>
							@endforeach
						</select>
					</div>
					...
					<div class="col-md-3">
						<select id="niveau_id" class="form-control" name="niveau_id" required>
							...
							<option @if($niveau->niveau_id == $niveau->niveau_id) selected @endif value="{{ $niveau->niveau_id }}">{{ $niveau->niveau_libelle }}</option>
							@endforeach
						</select>
					</div>
					@endif
