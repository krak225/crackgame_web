...
//SERVER-SIDE SCRIPTS FOR CHAP CHAP
  ...
  socket.on('ask-questionChap', function (data) {
	...
	//vérifier s'il n'a pas épuisé ses 10 question
	var sql_count_questions = 'SELECT count(*) as nbre_question_posee FROM chap_question where chap_id="'+data.chap_id+'" and repondeur_id="'+data.to_user_id+'"';
	db.query(sql_count_questions)
		.on('result', function(compteur){
			...
			if(compteur.nbre_question_posee < 10){
				...
				var sql = 'SELECT * FROM chap_score where chap_id="'+data.chap_id+'" and user_id="'+data.to_user_id+'"';
				console.log('sql: ',sql);
  ...
  socket.on('repondreChap', function (data) {
    ...
    var sql_info_chap = 'SELECT * FROM chap where chap_id="'+data.chap_id+'"';
	  db.query(sql_info_chap)
		.on('result', function(info_chap){

		var chap_etape = info_chap.chap_etape;
		//console.log('chap_etape', JSON.stringify(info_chap));
		...
		if(chap_etape!=""){
			...
			db.query(sql_selection_score)
				.on('result', function(info_chap_score_user){

				var cpt_question = info_chap_score_user.cpt_question;
				if(cpt_question <= 3){
					  ...
					  db.query(sql_question)
						...
						.on('end', function(){
							...
							.on('end', function(){
								...
								// console.log('resultatChap', JSON.stringify(resultatData));
...
server.listen(7070, function () {
  ...
  console.log('krak server listen at 7070');
