...
function confirmAction(texte,data){
	...
	noty({
		...
		buttons: [{
			addClass: 'btn btn-primary', text: 'Envoyer', onClick: function($noty) {
				...
				chat.handleSendInvitation(data);
...
function confirmDemarageChap(texte, data){
  ...
  noty({
    ...
    type: 'warning',
    buttons: [{
      addClass: 'btn btn-primary', text: 'Démarer', onClick: function($noty) {
        $noty.close();
        
        chat.handleDemarerChap(data);
...
var chat = {
    ...
    handleSendInvitation: function handleSendInvitation(data) {
    ...
    AccepterInvitation: function AccepterInvitation(data) {
        
    socket.emit('accepter-invitation', data);

    },

    handleDemarerChap: function DemarerChap(data) {
        
      socket.emit('demarer-chap', data);
...
$('document').ready(function(){
	...
	$('.btnSendInvitation').click(function(){
		...
		confirmAction('Voulez-vous vraiment envoyer une invitation?',data);
		
	});


  //Démarer le chap
  $('.btnDemarerChap').click(function(){
    
    var admin_id    = $(this).attr('data-admin_id')
    var chap_id = $(this).attr('data-chap_id');
    ...
    confirmDemarageChap('Voulez-vous vraiment démarer le Chap-chap ?',data);
...
socket.on('new_client_connected', function (data) {
		
	//Actualiser la liste des connectés
	$('#users_connected').load('http://changequiz.com/refreshduelistesconnectes');
	
});


socket.on('chapReady', function (data) {
  
  //vérifier s'il a souscrit au chap et le rediriger directement vers le chap en cours
  notification('LE CHAP-CHAP A DEMARE');
  location.href = 'http://changequiz.com/chapencours';
...
socket.on('invitation-accepte', function (data) {
	
	
	if(data.from_user_id == my_user_id){
		
		notification('Votre invitation a été acceptée');
    location.href = 'http://changequiz.com/duel/jouer/'+data.duel_id+'/'+data.to_user_id;
...
socket.on('send-invitation', function (data) {
	...
	//alert('to_user_id: '+data.to_user_id +' my_user_id: '+my_user_id);

	if(data.to_user_id == my_user_id){

		//Cacher le bouton inviter sur la ligne 
    $('.btnSendInvitation').each(function(){
      
        var el = $(this);

        if(el.attr('data-to_user_id') == data.from_user_id){

            el.replaceWith("Invitation reçu");

        }

    });
		//notification('Vous avez réçu une invitation');
		...
		noty({
			...
			modal: true,
			theme: 'defaultTheme',
			text: 'Vous avez réçu une invitation de ' + data.user_pseudo,
			type: 'warning',
			buttons: [{
				addClass: 'btn btn-primary', text: 'Accepter', onClick: function($noty) {
					 $noty.close();
					
					 chat.AccepterInvitation(data);

            //Added on 12052019 1700 : rediriger vers la page de jeu
            location.href = 'http://changequiz.com/duel/jouer/'+data.duel_id+'/'+data.from_user_id;
				
					}
				},
				{
					addClass: 'btn btn-warning', text: 'Refuser', onClick: function($noty) {
					$noty.close();
    ...
    $('.btnSendInvitation').each(function(){
        ...
        if(el.attr('data-to_user_id') == data.to_user_id){

            el.replaceWith("Invitation envoyé");

        }

    });

		notification('Votre invitation a été envoyée avec succès');
