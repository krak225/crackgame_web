 ...
 */
//var socket = __webpack_require__(28)('http://localhost:7070');
...
$('#btnInviter').click(function(){
	...
	noty({
		...
		modal: true,
		theme: 'defaultTheme',
		text:'Voulez-vous vraiment envoyer une invitation?',
		type: 'warning',
		buttons: [{addClass: 'btn btn-success ', text: 'OUI', onClick: function($noty) {
				$noty.close();
		   
				chat.handleInvitation(1, my_user_id);
...
var chat = {
    ...
    handleInvitation: fuinction handleInvitation(duel_id,my_user_id) {
    ...
    Abandonner: function ShowDialogInvitation(data) {
        
		alert("DIALOG INVITATION");
    ...
    loginUser: function loginUser(user_id,user_name) {
        ...
        socket.emit('user-join', {user_id:user_id});
...
socket.on('invitation', function (data) {
	...
	if(data.duel_id == duel_id){	
		chat.showDialogInvitation(data);
	}
