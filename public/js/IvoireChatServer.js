...
io.on('connection', function (socket) {
	...
	var sql_users_connected = 'select id,email from users where statut_connexion="CONNECTE"';
	
	db.query(sql_users_connected)
	.on('result', function(data){
		
		io.emit('new_client_connected', data);
		
		console.log("new_client_connected: ",JSON.stringify(data));
  ...
  socket.on('send-invitation', function (data) {
		...
		var sql_insert_msg = 'insert into message (`from_user_id`, `to_user_id`, `message_message`, `type`, `filename`) VALUES("'+data.from_user_id+'", "'+data.to_user_id+'", "'+data.message+'" , "'+data.type+'" , "'+data.filename+'" )';
		...
		console.log("send-invitation: ",sql_insert_msg);
...
//SERVER-SIDE SCRIPTS FOR CHAP CHAP
  ...
  socket.on('repondreChap', function (data) {
	...
	socket.on('send-message',function(data){
		...
		var sql_insert_msg = 'insert into message (`from_user_id`, `to_user_id`, `message_message`, `type`, `filename`, `message_date`) VALUES("'+data.from_user_id+'", "'+data.to_user_id+'", "'+data.message+'" , "'+data.type+'" , "'+data.filename+'" , "'+datetime_actuel+'" )';
...
server.listen(4000, function () {
  ...
  console.log('krak server listen at 4000');
