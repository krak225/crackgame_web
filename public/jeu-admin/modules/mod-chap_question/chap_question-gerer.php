<div class="leftbox" id="" style="padding:0px;">
		<div class="blocTitle">LISTE DES CHAP_QUESTIONS
		<a href="printer.php?table=chap_question" target="_blank"><img class="btn_modifier" src="images/ic/b_print.png" style="width:20px;cursor:pointer;position:relative;top:0px;float:right;margin:-5px 25px 5px 0px;color:black;font-weight:normal;" title="Imprimer"/></a>
...
<div>
	...
	if(isset($_POST["execAction"]))
	{
		...
		foreach($datas as $data){
			$x++;$table_id["id$x"]=$data->chap_question_id;}
		//pour chaque membre
		foreach($table_id as $id)
		{
			//définir la requète en fonction de l'action sélectionnée
			switch($action){
				case "activer":$sql='update chap_question set `chap_question_statut`="ACTIVE" where chap_question_id="'.$id.'"';break;
				case "desactiver":$sql='update chap_question set `chap_question_statut`="DESACTIVE" where chap_question_id="'.$id.'"';break;
				case "supprimer":$sql='delete from chap_question where chap_question_id="'.$id.'"';break;
			}
	...
	<select class="champ" name="repondeur_id" id="repondeur_id" title="Repondeur">
							<option value="">Repondeur</option>
							<?php 
							...
							$data = $stm->fetchAll(PDO::FETCH_OBJ);
							foreach($data as $d51){
								$selected = ($d51->repondeur_id==$repondeur_id)? ' selected ' : null;
								print '<option '.$selected.' value="'.$d51->repondeur_id.'">'.$d51->repondeur_id.'</option>';
							}
							?>
</select>
<input style="width:80px;" class="champ" type="text" name="reponse" id="reponse" placeholder="Reponse" title="Reponse"/>
<input style="width:80px;" class="champ" type="text" name="observation" id="observation" placeholder="Observation" title="Observation"/>
<input style="width:80px;" class="champ" type="text" name="statut" id="statut" placeholder="Statut" title="Statut"/>

			<select name="chap_question_statut" class="champ">
...
<th class="table-header-options line-left"><a href="">Repondeur</a></th>
<th class="table-header-options line-left"><a href="">Reponse</a></th>
<th class="table-header-options line-left"><a href="">Observation</a></th>
...
<th class="table-header-options line-left minwidth-4"><a href="">Action</a></th>
		...
		<?php
		$sql='select * from chap_question where 1 ';
		...
		$where = null; 
		 if(isset($_POST['chap_id']) and !empty($_POST['chap_id'])){$where.=' and chap_id="'.$_POST['chap_id'].'"';}
			 ...
			 if(isset($_POST['repondeur_id']) and !empty($_POST['repondeur_id'])){$where.=' and repondeur_id="'.$_POST['repondeur_id'].'"';}
			 if(isset($_POST['reponse']) and !empty($_POST['reponse'])){$where.=' and reponse="'.$_POST['reponse'].'"';}
			 if(isset($_POST['observation']) and !empty($_POST['observation'])){$where.=' and observation="'.$_POST['observation'].'"';}
			 if(isset($_POST['statut']) and !empty($_POST['statut'])){$where.=' and statut="'.$_POST['statut'].'"';}
			
		$sql.=$where;
		
		$sql.=' order by chap_question_id DESC '; 
		
		
		$nlpp=10;
		$url='chap_question.php?page=gerer';
		...
		print '<tr id="tr-'.$d->chap_question_id.'" '.$trclass.'>
			<td><input type="checkbox" class="chk" name="cfgM'.$d->chap_question_id .'" id="cfgM_'.$i .'"/></td>
			<td><a href="'.getPage().'.php?page=detail&id='.$d->chap_question_id.'">'.$d->chap_question_id .'</a></td>
			
			<td>'.stripslashes($d->chap_id).'</td>
			<td>'.stripslashes($d->question_id).'</td>
			<td>'.stripslashes($d->user_id).'</td>
			<td>'.stripslashes($d->repondeur_id).'</td>
			<td>'.stripslashes($d->reponse).'</td>
			<td>'.stripslashes($d->observation).'</td>
			<td>'.stripslashes($d->statut).'</td>
			<td><a href="'.getPage().'.php?page=modifier&id='.$d->chap_question_id.'"><img class="btn_modifier" src="images/btn_modifier.png" style="width:30px;" title="Modifier"/></a>
				<span style="cursor:pointer;" class="btn_suppr"><img class="btn_supprimer" id="'.$d->chap_question_id.'" data-table="chap_question" data-primarykey="chap_question_id"  data-value="'.$d->chap_question_id.'" src="images/btn_supprimer.png" style="width:;margin:3px;" title="Supprimer"/></span>	
			</td>
		</tr>';
		}
		?>
		<tr>
			...
			<th class="th" colspan="6" style="border:1px solid #d2d2d2;text-align:center;letter-spacing:5px;">
				ACTION
			</th>
