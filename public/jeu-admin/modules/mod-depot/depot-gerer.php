<div class="leftbox" id="" style="padding:0px;">
		<div class="blocTitle">LISTE DES DEPOTS
		<a href="printer.php?table=depot" target="_blank"><img class="btn_modifier" src="images/ic/b_print.png" style="width:20px;cursor:pointer;position:relative;top:0px;float:right;margin:-5px 25px 5px 0px;color:black;font-weight:normal;" title="Imprimer"/></a>
...
<div>
	...
	if(isset($_POST["execAction"]))
	{
		...
		foreach($datas as $data){
			$x++;$table_id["id$x"]=$data->depot_id;}
		//pour chaque membre
		foreach($table_id as $id)
		{
			//définir la requète en fonction de l'action sélectionnée
			switch($action){
				case "activer":$sql='update depot set `depot_statut`="ACTIVE" where depot_id="'.$id.'"';break;
				case "desactiver":$sql='update depot set `depot_statut`="DESACTIVE" where depot_id="'.$id.'"';break;
				case "supprimer":$sql='delete from depot where depot_id="'.$id.'"';break;
			}
...
<input style="width:80px;" class="champ" type="text" name="depot_montant" id="depot_montant" placeholder="Montant" title="Montant"/>
<input style="width:80px;" class="champ" type="text" name="depot_date" id="depot_date" placeholder="Date" title="Date"/>

			<select name="depot_statut" class="champ">
...
<th class="table-header-options line-left minwidth-4"><a href="">Action</a></th>
		...
		<?php
		$sql='select * from depot inner join users on users.id = depot.user_id where 1 ';
		
		$where = null; 
		 if(isset($_POST['depot_id']) and !empty($_POST['depot_id'])){$where.=' and depot_id="'.$_POST['depot_id'].'"';}
			 if(isset($_POST['user_id']) and !empty($_POST['user_id'])){$where.=' and user_id="'.$_POST['user_id'].'"';}
			 if(isset($_POST['depot_montant']) and !empty($_POST['depot_montant'])){$where.=' and depot_montant="'.$_POST['depot_montant'].'"';}
			 if(isset($_POST['depot_date']) and !empty($_POST['depot_date'])){$where.=' and depot_date="'.$_POST['depot_date'].'"';}
			
		$sql.=$where;
		
		$sql.=' order by depot_id DESC '; 
		
		
		$nlpp=10;
		$url='depot.php?page=gerer';
		...
		print '<tr id="tr-'.$d->depot_id.'" '.$trclass.'>
			<td><input type="checkbox" class="chk" name="cfgM'.$d->depot_id .'" id="cfgM_'.$i .'"/></td>
			<td><a href="'.getPage().'.php?page=detail&id='.$d->depot_id.'">'.$d->depot_id .'</a></td>
			
			<td>'.stripslashes($d->pseudo).'</td>
			<td>'.stripslashes($d->depot_montant).'</td>
			<td>'.stripslashes($d->depot_date).'</td>
			<td><a href="'.getPage().'.php?page=modifier&id='.$d->depot_id.'"><img class="btn_modifier" src="images/btn_modifier.png" style="width:30px;" title="Modifier"/></a>
				<span style="cursor:pointer;" class="btn_suppr"><img class="btn_supprimer" id="'.$d->depot_id.'" data-table="depot" data-primarykey="depot_id"  data-value="'.$d->depot_id.'" src="images/btn_supprimer.png" style="width:;margin:3px;" title="Supprimer"/></span>	
			</td>
		</tr>';
		}
		?>
