<div class="leftbox" id="" style="padding:0px;">
		<div class="blocTitle">LISTE DES CONVERSIONS
		<a href="printer.php?table=conversion" target="_blank"><img class="btn_modifier" src="images/ic/b_print.png" style="width:20px;cursor:pointer;position:relative;top:0px;float:right;margin:-5px 25px 5px 0px;color:black;font-weight:normal;" title="Imprimer"/></a>
...
<div>
	...
	if(isset($_POST["execAction"]))
	{
		...
		foreach($datas as $data){
			$x++;$table_id["id$x"]=$data->conversion_id;}
		//pour chaque membre
		foreach($table_id as $id)
		{
			//définir la requète en fonction de l'action sélectionnée
			switch($action){
				case "activer":$sql='update conversion set `conversion_statut`="ACTIVE" where conversion_id="'.$id.'"';break;
				case "desactiver":$sql='update conversion set `conversion_statut`="DESACTIVE" where conversion_id="'.$id.'"';break;
				case "supprimer":$sql='delete from conversion where conversion_id="'.$id.'"';break;
			}
...
<input style="width:80px;" class="champ" type="text" name="conversion_point" id="conversion_point" placeholder="Point" title="Point"/>
<input style="width:80px;" class="champ" type="text" name="conversion_taux" id="conversion_taux" placeholder="Taux" title="Taux"/>
...
<input style="width:80px;" class="champ" type="text" name="conversion_date" id="conversion_date" placeholder="Date" title="Date"/>

			<select name="conversion_statut" class="champ">
...
<th class="table-header-options line-left"><a href="">Point</a></th>
<th class="table-header-options line-left"><a href="">Taux</a></th>
<th class="table-header-options line-left"><a href="">Total_points_duel</a></th>
...
<th class="table-header-options line-left minwidth-4"><a href="">Action</a></th>
		...
		<?php
		$sql='select * from conversion inner join users on users.id = conversion.user_id  where 1 ';
		
		$where = null; 
		 if(isset($_POST['conversion_id']) and !empty($_POST['conversion_id'])){$where.=' and conversion_id="'.$_POST['conversion_id'].'"';}
			 if(isset($_POST['user_id']) and !empty($_POST['user_id'])){$where.=' and user_id="'.$_POST['user_id'].'"';}
			 if(isset($_POST['conversion_point']) and !empty($_POST['conversion_point'])){$where.=' and conversion_point="'.$_POST['conversion_point'].'"';}
			 if(isset($_POST['conversion_taux']) and !empty($_POST['conversion_taux'])){$where.=' and conversion_taux="'.$_POST['conversion_taux'].'"';}
			 if(isset($_POST['total_points_duel']) and !empty($_POST['total_points_duel'])){$where.=' and total_points_duel="'.$_POST['total_points_duel'].'"';}
			 if(isset($_POST['conversion_date']) and !empty($_POST['conversion_date'])){$where.=' and conversion_date="'.$_POST['conversion_date'].'"';}
			
		$sql.=$where;
		
		$sql.=' order by conversion_id DESC '; 
		
		
		$nlpp=10;
		$url='conversion.php?page=gerer';
		...
		print '<tr id="tr-'.$d->conversion_id.'" '.$trclass.'>
			<td><input type="checkbox" class="chk" name="cfgM'.$d->conversion_id .'" id="cfgM_'.$i .'"/></td>
			<td><a href="'.getPage().'.php?page=detail&id='.$d->conversion_id.'">'.$d->conversion_id .'</a></td>
			
			<td>'.stripslashes($d->pseudo).'</td>
			<td>'.stripslashes($d->conversion_point).'</td>
			<td>'.stripslashes($d->conversion_taux).'</td>
			<td>'.stripslashes($d->total_points_duel).'</td>
			<td>'.stripslashes($d->conversion_date).'</td>
			<td><a href="'.getPage().'.php?page=modifier&id='.$d->conversion_id.'"><img class="btn_modifier" src="images/btn_modifier.png" style="width:30px;" title="Modifier"/></a>
				<span style="cursor:pointer;" class="btn_suppr"><img class="btn_supprimer" id="'.$d->conversion_id.'" data-table="conversion" data-primarykey="conversion_id"  data-value="'.$d->conversion_id.'" src="images/btn_supprimer.png" style="width:;margin:3px;" title="Supprimer"/></span>	
			</td>
		</tr>';
		}
		?>
