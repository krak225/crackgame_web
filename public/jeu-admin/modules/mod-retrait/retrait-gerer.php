<div class="leftbox" id="" style="padding:0px;">
		<div class="blocTitle">LISTE DES RETRAITS
		<a href="printer.php?table=retrait" target="_blank"><img class="btn_modifier" src="images/ic/b_print.png" style="width:20px;cursor:pointer;position:relative;top:0px;float:right;margin:-5px 25px 5px 0px;color:black;font-weight:normal;" title="Imprimer"/></a>
...
<div>
	...
	if(isset($_POST["execAction"]))
	{
		...
		foreach($datas as $data){
			$x++;$table_id["id$x"]=$data->retrait_id;}
		//pour chaque membre
		foreach($table_id as $id)
		{
			//définir la requète en fonction de l'action sélectionnée
			switch($action){
				case "activer":$sql='update retrait set `retrait_statut`="ACTIVE" where retrait_id="'.$id.'"';break;
				case "desactiver":$sql='update retrait set `retrait_statut`="DESACTIVE" where retrait_id="'.$id.'"';break;
				case "supprimer":$sql='delete from retrait where retrait_id="'.$id.'"';break;
			}
...
<input style="width:80px;" class="champ" type="text" name="retrait_montant" id="retrait_montant" placeholder="Montant" title="Montant"/>
<input style="width:80px;" class="champ" type="text" name="retrait_date_demande" id="retrait_date_demande" placeholder="Date_demande" title="Date_demande"/>
<input style="width:80px;" class="champ" type="text" name="retrait_date_validation" id="retrait_date_validation" placeholder="Date_validation" title="Date_validation"/>

			<select name="retrait_statut" class="champ">
...
<th class="table-header-options line-left"><a href="">Date_demande</a></th>
...
<th class="table-header-options line-left minwidth-4"><a href="">Action</a></th>
		...
		<?php
		$sql='select * from retrait inner join users on users.id = retrait.user_id where 1 ';
		
		$where = null; 
		 if(isset($_POST['retrait_id']) and !empty($_POST['retrait_id'])){$where.=' and retrait_id="'.$_POST['retrait_id'].'"';}
			 if(isset($_POST['user_id']) and !empty($_POST['user_id'])){$where.=' and user_id="'.$_POST['user_id'].'"';}
			 if(isset($_POST['retrait_montant']) and !empty($_POST['retrait_montant'])){$where.=' and retrait_montant="'.$_POST['retrait_montant'].'"';}
			 if(isset($_POST['retrait_date_demande']) and !empty($_POST['retrait_date_demande'])){$where.=' and retrait_date_demande="'.$_POST['retrait_date_demande'].'"';}
			 if(isset($_POST['retrait_date_validation']) and !empty($_POST['retrait_date_validation'])){$where.=' and retrait_date_validation="'.$_POST['retrait_date_validation'].'"';}
			 if(isset($_POST['retrait_statut']) and !empty($_POST['retrait_statut'])){$where.=' and retrait_statut="'.$_POST['retrait_statut'].'"';}
			
		$sql.=$where;
		
		$sql.=' order by retrait_id DESC '; 
		
		
		$nlpp=10;
		$url='retrait.php?page=gerer';
		...
		print '<tr id="tr-'.$d->retrait_id.'" '.$trclass.'>
			<td><input type="checkbox" class="chk" name="cfgM'.$d->retrait_id .'" id="cfgM_'.$i .'"/></td>
			<td><a href="'.getPage().'.php?page=detail&id='.$d->retrait_id.'">'.$d->retrait_id .'</a></td>
			
			<td>'.stripslashes($d->pseudo).'</td>
			<td>'.stripslashes($d->retrait_montant).'</td>
			<td>'.stripslashes($d->retrait_date_demande).'</td>
			<td>'.stripslashes($d->retrait_date_validation).'</td>
			<td>'.stripslashes($d->retrait_statut).'</td>
			<td><a href="'.getPage().'.php?page=modifier&id='.$d->retrait_id.'"><img class="btn_modifier" src="images/btn_modifier.png" style="width:30px;" title="Modifier"/></a>
				<span style="cursor:pointer;" class="btn_suppr"><img class="btn_supprimer" id="'.$d->retrait_id.'" data-table="retrait" data-primarykey="retrait_id"  data-value="'.$d->retrait_id.'" src="images/btn_supprimer.png" style="width:;margin:3px;" title="Supprimer"/></span>	
			</td>
		</tr>';
		}
		?>
