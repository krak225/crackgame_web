<div class="leftbox" id="" style="padding:0px;">
		<div class="blocTitle">LISTE DES DUEL_JOCKERS
		<a href="printer.php?table=duel_jocker" target="_blank"><img class="btn_modifier" src="images/ic/b_print.png" style="width:20px;cursor:pointer;position:relative;top:0px;float:right;margin:-5px 25px 5px 0px;color:black;font-weight:normal;" title="Imprimer"/></a>
...
<div>
	...
	if(isset($_POST["execAction"]))
	{
		...
		foreach($datas as $data){
			$x++;$table_id["id$x"]=$data->duel_jocker_id;}
		//pour chaque membre
		foreach($table_id as $id)
		{
			//définir la requète en fonction de l'action sélectionnée
			switch($action){
				case "activer":$sql='update duel_jocker set `duel_jocker_statut`="ACTIVE" where duel_jocker_id="'.$id.'"';break;
				case "desactiver":$sql='update duel_jocker set `duel_jocker_statut`="DESACTIVE" where duel_jocker_id="'.$id.'"';break;
				case "supprimer":$sql='delete from duel_jocker where duel_jocker_id="'.$id.'"';break;
			}
...
<input style="width:80px;" class="champ" type="text" name="jocker_utilise" id="jocker_utilise" placeholder="Jocker_utilise" title="Jocker_utilise"/>

			<select name="duel_jocker_statut" class="champ">
...
<th class="table-header-options line-left"><a href="">Jocker_utilise</a></th>
<th class="table-header-options line-left minwidth-4"><a href="">Action</a></th>
		</tr>
		<?php
		$sql='select * from duel_jocker where 1 ';
		...
		$where = null; 
		 if(isset($_POST['duel_id']) and !empty($_POST['duel_id'])){$where.=' and duel_id="'.$_POST['duel_id'].'"';}
			 ...
			 if(isset($_POST['jocker_utilise']) and !empty($_POST['jocker_utilise'])){$where.=' and jocker_utilise="'.$_POST['jocker_utilise'].'"';}
			
		$sql.=$where;
		
		$sql.=' order by duel_jocker_id DESC '; 
		
		
		$nlpp=10;
		$url='duel_jocker.php?page=gerer';
		...
		print '<tr id="tr-'.$d->duel_jocker_id.'" '.$trclass.'>
			<td><input type="checkbox" class="chk" name="cfgM'.$d->duel_jocker_id .'" id="cfgM_'.$i .'"/></td>
			<td><a href="'.getPage().'.php?page=detail&id='.$d->duel_jocker_id.'">'.$d->duel_jocker_id .'</a></td>
			
			<td>'.stripslashes($d->duel_id).'</td>
			<td>'.stripslashes($d->user_id).'</td>
			<td>'.stripslashes($d->jocker_utilise).'</td>
			<td><a href="'.getPage().'.php?page=modifier&id='.$d->duel_jocker_id.'"><img class="btn_modifier" src="images/btn_modifier.png" style="width:30px;" title="Modifier"/></a>
				<span style="cursor:pointer;" class="btn_suppr"><img class="btn_supprimer" id="'.$d->duel_jocker_id.'" data-table="duel_jocker" data-primarykey="duel_jocker_id"  data-value="'.$d->duel_jocker_id.'" src="images/btn_supprimer.png" style="width:;margin:3px;" title="Supprimer"/></span>	
			</td>
		</tr>';
		}
		?>
		<tr>
			...
			<th class="th" colspan="2" style="border:1px solid #d2d2d2;text-align:center;letter-spacing:5px;">
				ACTION
			</th>
