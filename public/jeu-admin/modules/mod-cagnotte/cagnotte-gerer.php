<div class="leftbox" id="" style="padding:0px;">
		<div class="blocTitle">LISTE DES CAGNOTTES
		<a href="printer.php?table=cagnotte" target="_blank"><img class="btn_modifier" src="images/ic/b_print.png" style="width:20px;cursor:pointer;position:relative;top:0px;float:right;margin:-5px 25px 5px 0px;color:black;font-weight:normal;" title="Imprimer"/></a>
...
<div>
	...
	if(isset($_POST["execAction"]))
	{
		...
		foreach($datas as $data){
			$x++;$table_id["id$x"]=$data->cagnotte_id;}
		//pour chaque membre
		foreach($table_id as $id)
		{
			//définir la requète en fonction de l'action sélectionnée
			switch($action){
				case "activer":$sql='update cagnotte set `cagnotte_statut`="ACTIVE" where cagnotte_id="'.$id.'"';break;
				case "desactiver":$sql='update cagnotte set `cagnotte_statut`="DESACTIVE" where cagnotte_id="'.$id.'"';break;
				case "supprimer":$sql='delete from cagnotte where cagnotte_id="'.$id.'"';break;
			}
	...
	<form id="form-search" method="post" data-creator="kw-Builder" style="display:none;">
		<fieldset>
			<input style="width:80px;" class="champ" type="text" name="cagnotte_montant" id="cagnotte_montant" placeholder="Montant" title="Montant"/>
<input style="width:80px;" class="champ" type="text" name="cagnotte_date" id="cagnotte_date" placeholder="Date" title="Date"/>
<input style="width:80px;" class="champ" type="text" name="cagnotte_date_creation" id="cagnotte_date_creation" placeholder="Date_creation" title="Date_creation"/>

			<select name="cagnotte_statut" class="champ">
	...
	<table border="0" width="100%" cellpadding="0" cellspacing="0" id="product-table">
		...
		<?php
		$sql='select * from cagnotte where 1 ';
		
		$where = null; 
		 if(isset($_POST['cagnotte_id']) and !empty($_POST['cagnotte_id'])){$where.=' and cagnotte_id="'.$_POST['cagnotte_id'].'"';}
			 if(isset($_POST['cagnotte_montant']) and !empty($_POST['cagnotte_montant'])){$where.=' and cagnotte_montant="'.$_POST['cagnotte_montant'].'"';}
			 if(isset($_POST['cagnotte_date']) and !empty($_POST['cagnotte_date'])){$where.=' and cagnotte_date="'.$_POST['cagnotte_date'].'"';}
			 if(isset($_POST['cagnotte_date_creation']) and !empty($_POST['cagnotte_date_creation'])){$where.=' and cagnotte_date_creation="'.$_POST['cagnotte_date_creation'].'"';}
			
		$sql.=$where;
		
		$sql.=' order by cagnotte_id DESC '; 
		
		
		$nlpp=10;
		$url='cagnotte.php?page=gerer';
		...
		print '<tr id="tr-'.$d->cagnotte_id.'" '.$trclass.'>
			<td><input type="checkbox" class="chk" name="cfgM'.$d->cagnotte_id .'" id="cfgM_'.$i .'"/></td>
			<td>'.stripslashes($d->cagnotte_montant).'</td>
			<td>'.stripslashes($d->cagnotte_date).'</td>
			<td><a href="'.getPage().'.php?page=modifier&id='.$d->cagnotte_id.'"><img class="btn_modifier" src="images/btn_modifier.png" style="width:30px;" title="Modifier"/></a>
				<span style="cursor:pointer;" class="btn_suppr"><img class="btn_supprimer" id="'.$d->cagnotte_id.'" data-table="cagnotte" data-primarykey="cagnotte_id"  data-value="'.$d->cagnotte_id.'" src="images/btn_supprimer.png" style="width:;margin:3px;" title="Supprimer"/></span>	
			</td>
		</tr>';
		}
		?>
