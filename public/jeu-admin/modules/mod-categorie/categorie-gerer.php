<div class="leftbox" id="" style="padding:0px;">
		<div class="blocTitle">LISTE DES CATEGORIES
		<a href="printer.php?table=categorie" target="_blank"><img class="btn_modifier" src="images/ic/b_print.png" style="width:20px;cursor:pointer;position:relative;top:0px;float:right;margin:-5px 25px 5px 0px;color:black;font-weight:normal;" title="Imprimer"/></a>
...
<div>
	...
	if(isset($_POST["execAction"]))
	{
		...
		foreach($datas as $data){
			$x++;$table_id["id$x"]=$data->categorie_id;}
		//pour chaque membre
		foreach($table_id as $id)
		{
			//définir la requète en fonction de l'action sélectionnée
			switch($action){
				case "activer":$sql='update categorie set `categorie_statut`="ACTIVE" where categorie_id="'.$id.'"';break;
				case "desactiver":$sql='update categorie set `categorie_statut`="DESACTIVE" where categorie_id="'.$id.'"';break;
				case "supprimer":$sql='delete from categorie where categorie_id="'.$id.'"';break;
			}
	...
	<form id="form-search" method="post" data-creator="kw-Builder" style="display:none;">
		<fieldset>
			<input style="width:80px;" class="champ" type="text" name="categorie_libelle" id="categorie_libelle" placeholder="Libelle" title="Libelle"/>
<input style="width:80px;" class="champ" type="text" name="categorie_description" id="categorie_description" placeholder="Description" title="Description"/>

			<select name="categorie_statut" class="champ">
...
<th class="table-header-options line-left minwidth-4"><a href="">Action</a></th>
		...
		$where = null; 
		 if(isset($_POST['categorie_id']) and !empty($_POST['categorie_id'])){$where.=' and categorie_id="'.$_POST['categorie_id'].'"';}
			 if(isset($_POST['categorie_libelle']) and !empty($_POST['categorie_libelle'])){$where.=' and categorie_libelle="'.$_POST['categorie_libelle'].'"';}
			 if(isset($_POST['categorie_description']) and !empty($_POST['categorie_description'])){$where.=' and categorie_description="'.$_POST['categorie_description'].'"';}
			 if(isset($_POST['categorie_statut']) and !empty($_POST['categorie_statut'])){$where.=' and categorie_statut="'.$_POST['categorie_statut'].'"';}
			
		$sql.=$where;
		
		$sql.=' order by categorie_id DESC '; 
		
		
		$nlpp=10;
		$url='categorie.php?page=gerer';
		...
		print '<tr id="tr-'.$d->categorie_id.'" '.$trclass.'>
			<td><input type="checkbox" class="chk" name="cfgM'.$d->categorie_id .'" id="cfgM_'.$i .'"/></td>
			<td><a href="'.getPage().'.php?page=detail&id='.$d->categorie_id.'">'.$d->categorie_id .'</a></td>
			
			<td>'.stripslashes($d->categorie_libelle).'</td>
			<td>'.stripslashes($d->categorie_description).'</td>
			<td>'.stripslashes($d->categorie_statut).'</td>
			<td><a href="'.getPage().'.php?page=modifier&id='.$d->categorie_id.'"><img class="btn_modifier" src="images/btn_modifier.png" style="width:30px;" title="Modifier"/></a>
				<span style="cursor:pointer;" class="btn_suppr"><img class="btn_supprimer" id="'.$d->categorie_id.'" data-table="categorie" data-primarykey="categorie_id"  data-value="'.$d->categorie_id.'" src="images/btn_supprimer.png" style="width:;margin:3px;" title="Supprimer"/></span>	
			</td>
		</tr>';
		}
		?>
