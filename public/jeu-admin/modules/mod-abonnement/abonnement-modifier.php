<div class="leftbox" id="" style="padding:0px;">
			<div class="blocTitle">MODIFIER UN ABONNEMENT</div>
...
if(isset($_GET["id"]) and is_numeric($_GET["id"])){
	...
	$info='<div class="default-info">Renseignez tous les champs puis cliquez sur <b>Modifier</b> </div>';
		$user_id=stripcslashes($d->user_id);$type_jeu=stripcslashes($d->type_jeu);$abonnement_date=stripcslashes($d->abonnement_date);
		...
		if(isset($_POST["add"])){ 	
			...
			if($x->ToutEstCorrecte()){  
				$sql='UPDATE `abonnement` SET user_id = "'.$user_id.'",type_jeu = "'.$type_jeu.'",abonnement_date = "'.$abonnement_date.'"'; 
				 
				if(!empty($ext)){
					$sql.=',  ';
				}
				$sql.=' WHERE abonnement_id="'.$_GET["id"].'"';
