<div class="leftbox" id="" style="padding:0px;">
			<div class="blocTitle">DÉTAILS D'UN CHAP</div>
...
if(isset($_GET["id"]) and is_numeric($_GET["id"])){
	...
	<div><?php print $d->chap_participants;?></div>
	<label>Vainqueur</label>
	<div>
					<?php 
					$sql='select * from chap_vainqueur where chap_vainqueur_id="'.$d->chap_vainqueur_id.'"';
					//$req=mysql_query($sql);
					//$d1=mysql_fetch_object($req);
					...
					$d1 = $stm->fetchAll(PDO::FETCH_OBJ);
						...
						print '<div>'.$d1->chap_vainqueur_id.'</div>';
					
					?>
</div>
	<label>Date creation</label>
	<div><?php print $d->chap_date_creation;?></div>
