<div class="leftbox" id="" style="padding:0px;">
		<div class="blocTitle">LISTE DES DUEL_SCORES
		<a href="printer.php?table=duel_score" target="_blank"><img class="btn_modifier" src="images/ic/b_print.png" style="width:20px;cursor:pointer;position:relative;top:0px;float:right;margin:-5px 25px 5px 0px;color:black;font-weight:normal;" title="Imprimer"/></a>
...
<div>
	...
	if(isset($_POST["execAction"]))
	{
		...
		foreach($datas as $data){
			$x++;$table_id["id$x"]=$data->duel_score_id;}
		//pour chaque membre
		foreach($table_id as $id)
		{
			//définir la requète en fonction de l'action sélectionnée
			switch($action){
				case "activer":$sql='update duel_score set `duel_score_statut`="ACTIVE" where duel_score_id="'.$id.'"';break;
				case "desactiver":$sql='update duel_score set `duel_score_statut`="DESACTIVE" where duel_score_id="'.$id.'"';break;
				case "supprimer":$sql='delete from duel_score where duel_score_id="'.$id.'"';break;
			}
...
<input style="width:80px;" class="champ" type="text" name="connected" id="connected" placeholder="Connected" title="Connected"/>

			<select name="duel_score_statut" class="champ">
...
<th class="table-header-options line-left"><a href="">Connected</a></th>
<th class="table-header-options line-left minwidth-4"><a href="">Action</a></th>
		</tr>
		<?php
		$sql='select * from duel_score where 1 ';
		...
		$where = null; 
		 if(isset($_POST['duel_id']) and !empty($_POST['duel_id'])){$where.=' and duel_id="'.$_POST['duel_id'].'"';}
			 ...
			 if(isset($_POST['connected']) and !empty($_POST['connected'])){$where.=' and connected="'.$_POST['connected'].'"';}
			
		$sql.=$where;
		
		$sql.=' order by duel_score_id DESC '; 
		
		
		$nlpp=10;
		$url='duel_score.php?page=gerer';
		...
		print '<tr id="tr-'.$d->duel_score_id.'" '.$trclass.'>
			<td><input type="checkbox" class="chk" name="cfgM'.$d->duel_score_id .'" id="cfgM_'.$i .'"/></td>
			<td><a href="'.getPage().'.php?page=detail&id='.$d->duel_score_id.'">'.$d->duel_score_id .'</a></td>
			
			<td>'.stripslashes($d->duel_id).'</td>
			<td>'.stripslashes($d->user_id).'</td>
			<td>'.stripslashes($d->score).'</td>
			<td>'.stripslashes($d->connected).'</td>
			<td><a href="'.getPage().'.php?page=modifier&id='.$d->duel_score_id.'"><img class="btn_modifier" src="images/btn_modifier.png" style="width:30px;" title="Modifier"/></a>
				<span style="cursor:pointer;" class="btn_suppr"><img class="btn_supprimer" id="'.$d->duel_score_id.'" data-table="duel_score" data-primarykey="duel_score_id"  data-value="'.$d->duel_score_id.'" src="images/btn_supprimer.png" style="width:;margin:3px;" title="Supprimer"/></span>	
			</td>
		</tr>';
		}
		?>
