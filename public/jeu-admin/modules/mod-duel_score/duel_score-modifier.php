<div class="leftbox" id="" style="padding:0px;">
			<div class="blocTitle">MODIFIER UN DUEL_SCORE</div>
...
if(isset($_GET["id"]) and is_numeric($_GET["id"])){
	...
	$info='<div class="default-info">Renseignez tous les champs puis cliquez sur <b>Modifier</b> </div>';
		$user_id=stripcslashes($d->user_id);$score=stripcslashes($d->score);$connected=stripcslashes($d->connected);
		...
		if(isset($_POST["add"])){ 	
			...
			if($x->ToutEstCorrecte()){  
				$sql='UPDATE `duel_score` SET user_id = "'.$user_id.'",score = "'.$score.'",connected = "'.$connected.'"'; 
				 
				if(!empty($ext)){
					$sql.=',  ';
				}
				$sql.=' WHERE duel_score_id="'.$_GET["id"].'"';
