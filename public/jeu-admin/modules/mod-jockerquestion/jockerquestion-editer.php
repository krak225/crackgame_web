<div class="leftbox" id="" style="padding:0px;">
			<div class="blocTitle">ENREGISTRER UN JOCKERQUESTION</div>
	...
	$info='<div class="default-info">Renseignez tous les champs puis cliquez sur <b>Valider</b> </div>';
		...
		if(isset($_POST["add"])){ 
			...
			$jockerquestion_date= securisedData($jockerquestion_date);
				...
				$x->verifierNombre($beneficiaire_user_id,"Beneficiaire user",0,100000000000000000000000,1);
				$x->verifierNombre($jockerquestion_quantite,"Quantite",0,100000000000000000000000,1);
				$x->verifierNombre($jockerquestion_montant,"Montant",0,100000000000000000000000,1);
			
			if($x->ToutEstCorrecte()){  
				$sql='INSERT INTO `jockerquestion` (user_id,beneficiaire_user_id,jockerquestion_quantite,jockerquestion_montant,jockerquestion_date) 
				VALUES ("'.$user_id.'","'.$beneficiaire_user_id.'","'.$jockerquestion_quantite.'","'.$jockerquestion_montant.'","'.$jockerquestion_date.'")';
