<div class="leftbox" id="" style="padding:0px;">
			<div class="blocTitle">ENREGISTRER UN SOUSCRIPTION</div>
	...
	$info='<div class="default-info">Renseignez tous les champs puis cliquez sur <b>Valider</b> </div>';
		...
		if(isset($_POST["add"])){ 
			...
			$souscription_date= securisedData($souscription_date);
				...
				$x->verifierNombre($souscription_quantite,"Quantite",0,100000000000000000000000,1);
			
			if($x->ToutEstCorrecte()){  
				$sql='INSERT INTO `souscription` (user_id,beneficiaire_user_id,souscription_quantite,souscription_montant,souscription_date) 
				VALUES ("'.$user_id.'","'.$beneficiaire_user_id.'","'.$souscription_quantite.'","'.$souscription_montant.'","'.$souscription_date.'")';
